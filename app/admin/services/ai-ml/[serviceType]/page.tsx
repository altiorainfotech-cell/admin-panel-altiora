...
const defaultServiceData: AIMLServiceData = {
  serviceType: '',
  heroSection: {
    title: '',
    subtitle: '',
...
export default function AIMLServiceEditor() {
  const params = useParams()
  const router = useRouter()
  const { data: session } = useSession()
  const [serviceData, setServiceData] = useState<AIMLServiceData>(defaultServiceData)
  ...
  useEffect(() => {
    const fetchServiceData = async () => {
      ...
      try {
        const response = await fetch(`/api/admin/services/ai-ml/${serviceType}`)
        if (response.ok) {
          ...
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                {(serviceType === 'new' || serviceType === 'new-service') ? 'Create New AI/ML Service' : `Edit ${serviceType}`}
              </h1>
              <p className="text-gray-600 mt-1">AI/ML Services Management</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., machine-learning-consulting, ai-model-development"
                  disabled={!canWrite}
                />
              </div>
            </div>
